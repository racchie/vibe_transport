...
- [ ] 削除の「元に戻す」機能【保留・後日検討】
  - **判断:** 現時点では不要と判断。`confirm()` ダイアログで誤削除防止は十分。アプリ成熟度に応じて後日実装検討。
  - **実装案:** 削除操作後にトーストで "元に戻す" ボタン表示、一定時間内に復元可能にする
  - **参考:** 実装予定時間 1-2時間

## 未実装 / 推奨タスク（優先度：低）
  - スマホ版をベースとした設計をPC版でも見やすく・使いやすくするよう改善。
- [ ] アクセシビリティ改善（追加検討）
  - キーボードナビゲーション強化、フォーカス管理、セマンティック HTML の改善
  - トースト aria-live は既に実装済み
