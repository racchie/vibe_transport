...
- **アクセシビリティ強化（スクリーンリーダー対応）**
  ...
  - **入力バリデーション強化・駅名サジェスト**
    - フォームのリアルタイム/送信時バリデーションとエラーメッセージ表示
    - 運賃の警告: 100円未満/10,000円超で警告表示、50,000円を超えるとエラー
    - HeartRails API を使った駅名サジェスト（入力時の部分一致検索）
    - セッションキャッシュを導入し、検索負荷を軽減
    - 交通機関名の候補リスト（`datalist`）を導入、バス会社を充実
    - 候補の選択で会社名を自動補完（路線名から推測）
...
### Known Issues
- 運賃の自動取得は未対応（手動入力が必要）。有料API導入後に対応予定（Stage 2）。
- バス停のサジェスト/自動補完は未対応（HeartRailsは駅専用のため）。Stage 3で検討。
- スクリーンリーダー最適化の一部（`aria-describedby` や動的更新時の `aria-live` 通知）は次期で対応予定。
- E2E テスト（Playwright/Cypress）は未実装。回帰防止のため導入予定。
- オフライン時の駅名フォールバック辞書は未提供（API応答に依存）。
